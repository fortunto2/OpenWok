...
- [x] Task 2.2: Write unit tests for `SqliteRepo` — test each Repository method against an in-memory SQLite database with seeded test data. At minimum: list_restaurants (returns seeded), create_order (returns pricing breakdown), update_order_status (valid + invalid transitions), get_economics (aggregation).
