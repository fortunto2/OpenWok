...
- [x] Task 2.4: Refactor `crates/api/` — depend on `openwok-handlers`. In `main.rs`: create `SqliteRepo`, wrap in `Arc`, call `openwok_handlers::api_routes::<SqliteRepo>()`, merge with WebSocket route (stays in api). Remove old route modules (restaurants.rs, orders.rs, couriers.rs, economics.rs, metrics.rs) — keep only ws.rs and db.rs.
- [ ] Task 2.5: Run `make check` — all 37+ tests pass, clippy clean, fmt clean. Fix any compilation or test issues.
