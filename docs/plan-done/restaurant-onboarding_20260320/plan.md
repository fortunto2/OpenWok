...
## Phase 5: Review Fixes <!-- checkpoint:17fb4be -->

### Tasks
- [x] Task 5.1: Fix TOCTOU in `crates/handlers/src/restaurants.rs` `update_menu_item` handler — move `verify_ownership()` call BEFORE `repo.update_menu_item()`. Need to add `get_menu_item(id)` to Repository trait or query restaurant_id first. <!-- sha:17fb4be -->
- [x] Task 5.2: Fix TOCTOU in `crates/worker/src/lib.rs` `PATCH /api/menu-items/:id` route — same fix: verify ownership before applying update. <!-- sha:17fb4be -->
- [x] Task 5.3: Fix clippy errors in `crates/frontend/src/main.rs` — line 1471: remove `.clone()` on Copy type `Navigator`; line 1599: collapse nested if. <!-- sha:17fb4be -->
- [x] Task 5.4: Run `make check` to verify all fixes pass (tests + clippy + fmt). <!-- sha:17fb4be -->

### Verification
- [x] `make check` passes cleanly
- [x] TOCTOU fixed: non-owner update attempt returns 403 without modifying data
