...
## Phase 5: Review Fix Tasks (added by /review) <!-- checkpoint:f816233 -->

### Tasks
- [x] Task 5.1: Run `cargo fmt --all` to fix formatting across 8 files <!-- sha:cff69c5 -->
- [x] Task 5.2: Add auth enforcement to worker protected routes <!-- sha:f816233 -->
- [x] Task 5.3: Add payment flow to worker `POST /api/orders` <!-- sha:f816233 -->
- [x] Task 5.4: Fix unused import warning — remove `CreateCourierRequest` from test imports <!-- sha:f816233 -->
- [x] Task 5.5: Replace `.unwrap()` on `DateTime::parse_from_rfc3339()` with `.unwrap_or_default()` <!-- sha:f816233 -->

### Verification
- [x] `cargo test` passes (91 tests, 0 failures)
- [x] `cargo clippy -- -D warnings` passes (0 warnings)
- [x] `cargo fmt --all --check` passes
- [x] Worker builds for wasm32-unknown-unknown
- [x] Worker protected routes call `extract_auth()` — return 401 without valid JWT
- [x] Worker POST /api/orders creates Payment record + Stripe Checkout Session
