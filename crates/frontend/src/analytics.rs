...
// PostHog analytics — only available on WASM (JS interop).
...
pub fn posthog_capture(_event: &str) {
    // No-op on native — PostHog JS SDK not available
}
...
#[cfg(not(target_arch = "wasm32"))]
pub fn posthog_capture_with_props(_event: &str, _props: &serde_json::Value) {
    // No-op on native
}
