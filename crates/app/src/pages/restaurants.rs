...
pub fn RestaurantList() -> Element {
    let restaurants = use_server_future(crate::server_fns::restaurants::get_restaurants)?;
...
pub fn RestaurantMenu(id: String) -> Element {
    let restaurant = use_server_future(move || {
        let id = id.clone();
        async move { crate::server_fns::restaurants::get_restaurant(id).await }
    })?;
...
fn CartPanel() -> Element {
    ...
    let total = state
        .items
        .iter()
        .fold(openwok_core::money::Money::from("0"), |acc, i| {
            acc + i.price * Decimal::from(i.quantity)
        });
